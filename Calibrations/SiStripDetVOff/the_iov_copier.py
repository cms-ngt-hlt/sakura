...
########################################################################
TAG_MAIN = "SiStripDetVOff_1hourDelay_v1_Validation"
TAG_REF = "SiStripDetVOff_NGTDemonstrator"
OBJ_TIPE = "SiStripDetVOff"
LS_DURATION = 23.3  # seconds
STATE_FILE = "/afs/cern.ch/user/s/sakura/uploaded_SiStripDetVOff_payloads.txt"
########################################################################
...
def parse_conddb_list(output):
    ...
    for line in output.splitlines():
        m = re.match(r"(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}).*\s([0-9a-f]{40})\s+SiStripDetVOff", line)
        if m:
...
def parse_conddb_list_main(output):
    ...
    for line in output.splitlines():
        m = re.match(
            r"(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}) \((\d+)\).*?([0-9a-f]{40})\s+SiStripDetVOff",
            line,
        )
        if m:
...
def parse_conddb_list_inrun(output):
    ...
    entries = []
    for line in output.splitlines():
        # Match e.g. "398421  2025-10-24 14:13:57  7a38b1...  SiStripDetVOff"
        m = re.match(
            r"(\d+)\s+\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}\s+([0-9a-f]{40})\s+SiStripDetVOff",
            line,
        )
        if m:
...
def parse_conddb_list_inrunls(output):
    ...
    for line in output.splitlines():
        # Example line:
        # 398515 :   1326 (1711608891966766)  2025-10-27 16:39:52  26b2f052da7fbe7c4884d4ed875e027f9885e799  SiStripDetVOff
        m = re.match(
            r"\s*(\d+)\s*:\s*(\d+)\s*\((\d+)\)\s+\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}\s+([0-9a-f]{40})\s+SiStripDetVOff",
            line,
        )
        if m:
...
def read_last_uploaded():
    ...
    last_line = lines[-1]
    idx_str, payload_hash = last_line.split()
    return int(idx_str), payload_hash

def append_current_last(iov_main):
    if not iov_main:
        return

    last_index = len(iov_main) - 1
    last_hash = iov_main[last_index][2]  # assuming (since, till, hash)

    with open(STATE_FILE, "a") as f:
        f.write(f"{last_index} {last_hash}\n")

    print(f"Recorded new last index: {last_index} ({last_hash})")
...
last_uploaded_index, last_uploaded_hash = read_last_uploaded()

if last_uploaded_index is None:
    #print("No state file found: taking all payloads.")
    #new_iovs = iov_main
    raise RuntimeError(
        "No state file ("+STATE_FILE+") found!!!! Do not dare to do anything!" 
    )        
else:
    print(f"Last uploaded index: {last_uploaded_index}")
    ## sanity check
    if iov_main[last_uploaded_index][2] != last_uploaded_hash:
        raise RuntimeError(
            "State file mismatch: stored hash ("+ last_uploaded_hash+") does not match main tag at that index ("+iov_main[last_uploaded_index][2]+")"
        )    
    new_iovs = iov_main[last_uploaded_index + 1:]

print(f"Found {len(new_iovs)} new payload(s) to process.")
...
# --- now update the state file with the last IOV
append_current_last(iov_main)
